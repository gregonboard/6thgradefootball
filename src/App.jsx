...
function genPlayElements(conceptKey, spots, dir, tags = [], formation) {
  ...
  switch (conceptKey) {
    ...
    case "sparrow":
      ...
      rt("X", [[0, -9], [1.5, -7]]);
      rt("Z", [[0, -9], [1.5, -7]]);
      rt("H", [[0, -4], [8, -10]]); /* slant: Greg's ruling, the out collided with X's hitch */
      rt("Y", [[0, -8], [3, -7]]);
      rbLeak();
      throwTo(s > 0 ? "Z" : "X");
      break;
    ...
    case "flood": {
      ...
      /* Z runs the POST-CORNER on every flood (Greg, Sept 3): stem, break to the
         post, then break back to the corner and deep. X keeps his side job: the
         GO on the call side to pull the top off, the post from the backside. */
      if (has("Z")) { const [x, y] = at("Z"); const m = x <= 50 ? 1 : -1; add("Z", "route", [[x, y], [x, y - 8], [x + m * 7, y - 14], [x + m * 3, y - 22]]); }
      if (has(goSide) && goSide !== "Z") add(goSide, "route", [at(goSide), [at(goSide)[0], at(goSide)[1] - 19]]);
      if (has(backSide) && backSide !== "Z") add(backSide, "route", [at(backSide), [at(backSide)[0], at(backSide)[1] - 8], [at(backSide)[0] + s * 9, at(backSide)[1] - 15]]);
    ...
    case "bubble": {
      ...
      const T = outsideAt(s);
      const B = outsideAt(-s);
      /* smoke: one step back, then north; no lateral drift for a corner to jump */
      if (has(T)) add(T, "route", [at(T), [at(T)[0], at(T)[1] + 2.5], [at(T)[0] + s * 1.5, at(T)[1] - 5]]);
      if (has(B)) add(B, "block", [at(B), [at(B)[0], at(B)[1] - 4]]);
      if (has("Y")) add("Y", "block", [at("Y"), [at("Y")[0] + s * 3, at("Y")[1] - 5]]);
      jetMotion();
      if (has("RB")) { if (rbInBackfield) add("RB", "fake", [at("RB"), [50 + s * 4, 28]]); else add("RB", "block", [at("RB"), [at("RB")[0], at("RB")[1] - 4]]); }
      throwTo(T, 2);
      break;
    }
    case "slip":
...
function DefDiagram({ front, cov, stunt, blitz, size = "big" }) {
  ...
  return (
    <svg className={"play-svg " + size} viewBox="0 0 100 44" preserveAspectRatio="xMidYMid meet">
      ...
      {off.map((o, i) => (
        <g key={"o" + i}>
          <circle cx={o.x} cy="24" r="2.1" fill="#EDEDEA" stroke="#B9BCC2" strokeWidth="0.4" />
          <text x={o.x} y="24.8" textAnchor="middle" fontSize="2.2" fontWeight="700" fill="#9AA0A8" fontFamily="Roboto, sans-serif">{o.l}</text>
        </g>
      ))}
      <circle cx="50" cy="30" r="2.1" fill="#EDEDEA" stroke="#B9BCC2" strokeWidth="0.4" /><text x="50" y="30.8" textAnchor="middle" fontSize="2.2" fontWeight="700" fill="#9AA0A8" fontFamily="Roboto, sans-serif">Q</text>
    ...
    <div className="two-col">
      <section className="panel">
        <div className="panel-head">
          <h2>Defense</h2>
          <button className="btn" onClick={onPrint}>Print Defense Sheet</button>
        </div>
      ...
      <section className="panel">
        ...
        <div className="def-rules">
          {DEF_RULES.map((r) => (
            <div key={r.n} className="def-rule"><b>{r.n}</b><div><b>{r.t}</b><span>{r.d}</span></div></div>
          ))}
        </div>
      </section>
    </div>
...
const GAME_PLANS = [
  {
    ...
    keys: [
      ...
      "THEY GET UPFIELD FAST: down blocks use their charge, reach blocks lose to it, so no Ram or Leopard this week. HAMMER is the base. LINE RULE OF THE WEEK vs the head-up nose: playside guard takes the NOSE, the CENTER blocks BACK for the blitzer. Backside tackle walls the man over the pulled guard. Rhino and Lion lead the game.",
    ...
    sheet: {
      ...
      run: ["Doubles · Rhino", "Doubles · Lion", "Doubles · Rocket", "Doubles Lt · Laser", "Doubles · Raccoon", "Doubles · Longhorn", "Doubles · Rabbit", "Doubles · Lynx", "Empty · Rocket", "Empty Lt · Laser", "Empty · Raccoon", "Empty Lt · Longhorn", "Nasty Lt · Lion", "Nasty Rt · Rhino"],
      pass: ["Doubles · Robin", "Doubles · Sparrow", "Doubles · Owl", "Doubles · Lion Owl", "Doubles · Raven", "Doubles · Lark", "Doubles · Rhino Heron", "Doubles · Lion Heron", "Doubles · Eagle", "Empty · Robin", "Empty · Laffy", "Empty · Reese's"],
  ...
  },
  {
    ...
    keys: [
      "THOMPSON 4-4: eight in the box, one safety, and the corner on the tight side creeps in as a ninth. They will not honor the wide X. Cushion over X = candy: Laffy from Doubles or Nasty Rt, and the Now reads are free all night.",
      "THEIR RIGHT END lines up over OUR LEFT tackle: fast, crashes inside. LT rule all night: the end is coming to you, block HIM first, never step past him. Tank Lt Lion and Nasty Lt Lion put three blockers on that edge. Everything outside him is open: Leopard, Laser, Longhorn, and LARK sprints right around a man who runs himself inside.",
      "THEIR LEFT INSIDE BACKER lines up over OUR RIGHT guard: 60% of their tackles. Left is home: Lion, Leopard, Lynx, Lizard, Longhorn all run away from him. Run at him only with a wall (Tank Rt Rhino, Nasty Rt Rhino). Make him wrong: Owl and Lion Owl pop Y behind him the snap after he fills.",
      "NASTY RUNS IT, SPEED THROWS IT: Z is a wing in Super Heavy, so the only Nasty throws are Y's Owls (same kid) and X's smoke. Every other bird comes from Doubles or Trips with the catchers on the field. Speed swaps one kid (the wing for Z).",
      "THE SHOT: Speed personnel, Doubles Eagle, called right after a heavy drive while the safety is still thinking run. X post behind him, Z go clears the corner, Y drags. One look, then the drag.",
      "SUPER HEAVY IS NEW THIS WEEK: Doubles is still home and Tank is still the hammer. Nasty gets a drive at a time (twelve calls, all words they know), never the whole game. Heavy only at the sticks and the goal line. Backup H leads and blocks first (Leopard, Lion), then gets his jet. Up two scores in the 4th: milk it.",
    ],
    /* balance (Greg, Sept 1): Speed is home, Tank is the known hammer, Nasty is
       the new install and gets a tight twelve-call package. Openers are four
       Doubles calls the kids own cold, then Nasty enters once the box is loaded. */
    sheet: {
      openers: ["Doubles · Rhino", "Doubles · Laffy", "Doubles · Lion", "Doubles · Owl", "Nasty Lt · Lion", "Nasty Lt · Leopard"],
      /* left (away from the freak, at the crasher) listed first; each word's mirror beside it */
      run: ["Doubles · Lion", "Doubles · Rhino", "Tank Lt · Lion", "Tank Rt · Rhino", "Nasty Lt · Lion", "Nasty Rt · Rhino", "Tank Lt · Leopard", "Tank Rt · Ram", "Nasty Lt · Leopard", "Nasty Rt · Ram", "Nasty Lt · Laser", "Nasty Rt · Rocket", "Nasty Lt · Longhorn", "Nasty Rt · Raccoon", "Doubles · Lynx", "Doubles · Lizard"],
      pass: ["Doubles · Owl", "Nasty Rt · Owl", "Nasty Rt · Lion Owl", "Doubles · Rhino Now", "Doubles · Lion Now", "Doubles · Eagle", "Doubles · Sparrow", "Doubles · Robin", "Trips Rt · Hawk", "Doubles · Reese's"],
      third_short: ["I Rt · Moose", "Tank Rt · Moose", "Nasty Lt · Lynx", "I Lt · Lion", "Tank Lt · Lion", "Doubles · Lynx"],
      ...
      redzone: ["Tank Lt · Lion", "Nasty Lt · Lion", "Doubles · Owl", "Tank Rt · Owl", "Doubles · Laffy", "Nasty Rt · Laffy", "Doubles · Lizard", "Doubles · Eagle"],
      goalline: ["I Rt · Moose", "I Lt · Lion", "Tank Lt · Lion", "Tank Rt · Owl", "I Rt · Rhino", "Tank Rt · Rhino"],
      special: ["Doubles · Loop", "Doubles · Rainbow"],
    },
  },
];
...
/* ---- route cards print ---- */
.def-controls { padding: 12px 16px; display: grid; gap: 10px; border-bottom: 1px solid var(--line); background: #F6F4EF; }
.def-row { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
.def-lbl { font-family: var(--disp); font-weight: 700; font-size: 12px; letter-spacing: 1px; text-transform: uppercase; color: var(--muted); width: 70px; }
.def-chip { appearance: none; border: 1.5px solid var(--line); background: #fff; font-family: var(--disp); font-weight: 600; font-size: 13px; padding: 6px 12px; cursor: pointer; border-radius: 6px; }
.def-chip.on { background: var(--ink); border-color: var(--ink); color: #fff; }
.def-call { padding: 10px 16px; font-family: var(--disp); font-size: 15px; letter-spacing: .5px; }
.def-call b { color: var(--red); }
.def-diagram { padding: 8px 16px; }
.def-jobs { padding: 8px 16px; display: grid; gap: 10px; }
.def-job { display: grid; gap: 2px; }
.def-job b { font-family: var(--disp); letter-spacing: .5px; }
.def-job span { font-size: 13px; color: var(--muted); line-height: 1.4; }
.def-rules { padding: 8px 16px 16px; display: grid; gap: 8px; }
.def-rule { display: flex; gap: 10px; align-items: flex-start; }
.def-rule > b { font-family: var(--disp); font-weight: 700; font-size: 22px; line-height: 1; color: var(--red); flex-shrink: 0; }
