...
describe("vocabulary", () => {
  ...
  it("builds the QB tree: sprint-out flood exists with a run answer", () => {
    ...
    expect(SEED.plays.length).toBe(100);
  });
...
describe("seeds", () => {
  ...
  it("seeds the v6 costume looks", () => {
    ...
    expect(SEED.plays.length).toBe(100); // 71 after the Aug 17 cuts + the 14 Nasty looks (v15) // the v13 weaponized layer (5) + the two Orbits (v21) + Empty Eagle (v22)
  });
  ...
  it("migrates a v3 program to v4 once, without duplicates", () => {
    ...
    expect(v3.plays.length).toBe(100); // everything a fresh install gets, no dupes
    expect(v3.safariVersion).toBe(22);
    ...
    expect(again.plays.length).toBe(100);
...
describe("normalizeData migration", () => {
  it("upgrades an old save: kills paired, packages seeded, names re-derived, plan appended", () => {
    ...
    expect(d.safariVersion).toBe(22);
  });
...
describe("formations", () => {
  it("staggers formation installs on the week dial", () => {
    ...
    expect(installedForms(6).length).toBe(15); // 12 + the I Rt/Lt rain package + Empty Lt
  ...
  it("keeps every formation legal: exactly OL + X + Y on the line, Z and H off", () => {
    for (const f of ["Doubles", "Doubles Lt", "Trips Rt", "Trips Lt", "Bunch Rt", "Bunch Lt", "Stack", "Nasty Rt", "Nasty Lt", "Empty", "Empty Lt", "Tank Rt", "Tank Lt"]) {
...
describe("app end-to-end", () => {
  ...
  it("Empty allows the QUICK family plus jets and keeps, never a dropback or a puller", async () => {
    await load();
    fireEvent.click(screen.getByText("Play Lab"));
    ...
    expect(opts.some((t) => /Hawk|Falcon/.test(t))).toBe(false);
    // Eagle is the one dropback Greg allowed from Empty on purpose (Sept 22)
    expect(opts.some((t) => /Eagle/.test(t))).toBe(true);
    expect(opts.some((t) => /Reese's/.test(t))).toBe(true);
    expect(opts.some((t) => /Rocket \/ Laser/.test(t))).toBe(true);
    expect(opts.some((t) => /Raccoon \/ Longhorn/.test(t))).toBe(true);
    expect(screen.getByText(/QUICK family plus jets and keeps/)).toBeTruthy();
  });
...
describe("super heavy (Sept 1)", () => {
  ...
  it("v15: the Nasty install lands once on an existing v14 program, deduped by name", () => {
    ...
    expect(v14.safariVersion).toBe(22);
...
describe("Heron (Greg, Sept 7): H's ball off the hammer", () => {
  const kinds = (el, L) => (el[L] || []).map((e) => e.kind);
  it("every H alignment gets the same job: fake the block, leak to the flat, turn up", () => {
    /* Doubles: H is backside on Rhino, so he motions across, fakes, leaks */
    const dbl = genPlayElements("power", formSpots("Doubles"), "Rt", ["Heron"], "Doubles");
    expect(kinds(dbl, "H")).toEqual(["motion", "fake", "route"]);
    /* Trips Rt with Rhino: H already play side, no motion */
    const tr = genPlayElements("power", formSpots("Trips Rt"), "Rt", ["Heron"], "Trips Rt");
    expect(kinds(tr, "H")).toEqual(["fake", "route"]);
    /* Tank: shuffle, fake the kick, leak */
    const tank = genPlayElements("power", formSpots("Tank Rt"), "Rt", ["Heron"], "Tank Rt");
    expect(kinds(tank, "H")).toEqual(["motion", "fake", "route"]);
    /* I: the fullback steps into the hole then out to the flat */
    const iform = genPlayElements("power", formSpots("I Rt"), "Rt", ["Heron"], "I Rt");
    expect(kinds(iform, "H")).toEqual(["fake", "route"]);
    /* Nasty Lt Lion: H is the backside wing, motions like Lion; Nasty Lt Rhino: playside wing, no motion */
    expect(kinds(genPlayElements("power", formSpots("Nasty Lt"), "Lt", ["Heron"], "Nasty Lt"), "H")).toEqual(["motion", "fake", "route"]);
    expect(kinds(genPlayElements("power", formSpots("Nasty Lt"), "Rt", ["Heron"], "Nasty Lt"), "H")).toEqual(["fake", "route"]);
    for (const el of [dbl, tr, tank, iform]) {
      const route = el.H.find((e) => e.kind === "route").pts;
      expect(route[route.length - 1][1]).toBeLessThan(route[0][1]); /* ends upfield */
      expect(kinds(el, "RB")).toEqual(["fake", "block"]);            /* RB fakes, then owns the end */
      expect(el.RB.some((e) => e.kind === "carry")).toBe(false);
      expect(kinds(el, "QB")).toContain("throw");
      const throwPt = el.QB.find((e) => e.kind === "throw").pts[1];
      expect(route.some(([x, y]) => x === throwPt[0] && y === throwPt[1])).toBe(true); /* the throw lands on H's route */
    }
    /* the play-side outside man clears; on Lion from Doubles that is X */
    const lion = genPlayElements("power", formSpots("Doubles"), "Lt", ["Heron"], "Doubles");
    expect(kinds(lion, "X")).toEqual(["route"]);
    expect(kinds(lion, "Z")).toEqual(["block"]);
    expect(kinds(dbl, "Z")).toEqual(["route"]);
    /* the line still blocks HAMMER: backside guard pulls */
    expect(kinds(dbl, "LG")).toEqual(["route"]);
    /* Heron means nothing on a non-power word */
    expect(genPlayElements("jet", formSpots("Doubles"), "Rt", ["Heron"], "Doubles").H.some((e) => e.kind === "carry")).toBe(true);
  });
  it("cards, carrier, type, and seeds", () => {
    const d = normalizeData({ seasonWeek: 9 });
    const rh = d.plays.find((p) => p.name === "Doubles · Rhino Heron");
    const lh = d.plays.find((p) => p.name === "Doubles · Lion Heron");
    expect(rh && lh).toBeTruthy();
    expect(rh.type).toBe("Pass");
    expect(playCarrier(rh)).toBe("H");
    expect(callWord("power", "Rt", ["Heron"])).toBe("Rhino Heron");
    const jobs = jobsFor(rh);
    expect(jobs.H).toMatch(/HERON is YOUR ball/);
    expect(jobs.RB).toMatch(/without the ball/);
    expect(jobs.OL).toBe(ASSIGNMENTS.power.OL);
    expect(jobsFor({ concept: "power", dir: "Rt", formation: "I Rt", tags: ["Heron"] }).H).toMatch(/C gap/);
    expect(jobsFor({ concept: "power", dir: "Rt", formation: "Nasty Lt", tags: ["Heron"] }).H).toMatch(/HERON is YOUR ball/);
    /* v17 lands once on an existing v16 program */
    const v16 = normalizeData({ safariVersion: 16, plays: SEED.plays.filter((p) => !/Heron/.test(p.name)) });
    expect(v16.plays.filter((p) => /Heron/.test(p.name)).length).toBe(6);
    for (const n of ["Tank Rt · Rhino Heron", "Tank Lt · Lion Heron", "I Rt · Rhino Heron", "I Lt · Lion Heron"]) expect(v16.plays.some((p) => p.name === n), n).toBe(true);
    expect(v16.plays.find((p) => p.name === "I Rt · Rhino Heron").type).toBe("Pass");
    expect(normalizeData(JSON.parse(JSON.stringify(v16))).plays.length).toBe(v16.plays.length);
  });
});

describe("Sept 9: the sweep is blocked, Empty flexes Y, the Red plan", () => {
  const kinds = (el, L) => (el[L] || []).map((e) => e.kind);
  it("Y owns the end man on every sweep and keep; the RB leads instead of faking", () => {
    /* Doubles Rocket: Y is tight, so he HOOKS the end (first step outside, then up) */
    const rk = genPlayElements("jet", formSpots("Doubles"), "Rt", [], "Doubles");
    const y = rk.Y.find((e) => e.kind === "block").pts;
    expect(y[1][0]).toBeGreaterThan(y[0][0]);
    expect(y[y.length - 1][1]).toBeLessThan(y[0][1]);
    expect(kinds(rk, "RB")).toEqual(["block"]);           /* no more fake away */
    const rb = rk.RB[0].pts;
    expect(rb[rb.length - 1][0]).toBeGreaterThan(70);      /* he leads all the way to the alley */
    ...
    expect(el.H.find((e) => e.kind === "motion").pts[0][0]).toBeGreaterThan(50);
    expect(el.Y.find((e) => e.kind === "block").pts[1][0]).toBeGreaterThan(el.Y[0].pts[0][0]);
    /* keeps share the edge: Y block, RB lead */
    const keep = genPlayElements("keep", formSpots("Doubles"), "Rt", [], "Doubles");
    expect(kinds(keep, "Y")).toEqual(["block"]);
    expect(kinds(keep, "RB")).toEqual(["block"]);
    /* Rocket Owl still puts Y on the seam */
    expect(genPlayElements("jet", formSpots("Doubles"), "Rt", ["Owl"], "Doubles").Y.some((e) => e.kind === "carry")).toBe(true);
    /* the words say so */
    expect(ASSIGNMENTS.jet.Y).toMatch(/END MAN/);
    expect(ASSIGNMENTS.jet.RB).toMatch(/LEAD/);
    expect(ASSIGNMENTS.keep.Y).toMatch(/END MAN/);
  });
  it("Empty flexes Y off the tackle and stays legal; Empty Lt mirrors it", () => {
    const e = formSpots("Empty"), l = formSpots("Empty Lt");
    expect(e.Y[0] - e.RT[0]).toBeGreaterThan(8);
    expect(e.Y[1]).toBe(23);
    expect(e.Z[0]).toBeGreaterThan(e.Y[0]);
    expect(l.Y[0]).toBe(100 - e.Y[0]);
    expect(l.H[0]).toBeGreaterThan(50);
    expect(FORM_WEEKS["Empty Lt"]).toBe(6);
    expect(installedForms(6)).toContain("Empty Lt");
  });
  it("v19 seeds the Empty sweeps once", () => {
    const d = normalizeData({ safariVersion: 18, plays: SEED.plays.filter((p) => !/^Empty( Lt)? · (Rocket|Laser|Raccoon|Longhorn)$/.test(p.name)) });
    for (const n of ["Empty · Rocket", "Empty Lt · Laser", "Empty · Raccoon", "Empty Lt · Longhorn"]) expect(d.plays.filter((p) => p.name === n).length, n).toBe(1);
    expect(normalizeData(JSON.parse(JSON.stringify(d))).plays.length).toBe(d.plays.length);
  });
  it("the Red plan resolves, opens in Doubles, uses no pullers except the trap, no Moose", () => {
    const d = normalizeData({ seasonWeek: 9 });
    const plan = GAME_PLANS.find((g) => g.key === "red");
    const byName = Object.fromEntries(d.plays.map((p) => [p.name, p]));
    for (const list of Object.values(plan.sheet)) for (const n of list) expect(byName[n], n).toBeTruthy();
    const patch = applyGamePlan(d, plan);
...
/* ---------- Empty Eagle: the rule Greg broke on purpose (Sept 22) ---------- */
describe("empty eagle", () => {
  it("routes the RB instead of blocking him, because nobody is home", () => {
    const empty = genPlayElements("eagle", formSpots("Empty"), "", [], "Empty");
    const doubles = genPlayElements("eagle", formSpots("Doubles"), "", [], "Doubles");
    // in Doubles the RB stays in and protects
    expect(doubles.RB.map((e) => e.kind)).toEqual(["block"]);
    // from Empty he is a slot, so he runs: a blocking slot would be a lie on the card
    expect(empty.RB.some((e) => e.kind === "block")).toBe(false);
    expect(empty.RB.some((e) => e.kind === "route")).toBe(true);
    // the shot itself is unchanged: X post, Z go, Y drag, H bubble
    expect(empty.Z.some((e) => e.kind === "route")).toBe(true);
    expect(empty.X.some((e) => e.kind === "route")).toBe(true);
  });

  it("tells the truth on the cards: five protect, ball out on one count", () => {
    const jobs = jobsFor({ formation: "Empty", concept: "eagle", dir: "", tags: [] });
    expect(jobs.OL).toMatch(/FIVE of you/);
    expect(jobs.QB).toMatch(/ONE count/);
    expect(jobs.RB).toMatch(/SLOT, not a blocker/);
    // every other formation keeps the normal Eagle cards
    expect(jobsFor({ formation: "Doubles", concept: "eagle", dir: "", tags: [] })).toEqual(ASSIGNMENTS.eagle);
  });

  it("v22 seeds Empty Eagle once on a v21 program", () => {
    const v21 = { players: [], safariVersion: 21, day1Seeded: true, week2Seeded: true, savedPlans: [], plays: SEED.plays.filter((p) => p.name !== "Empty · Eagle").map((p) => ({ ...p })) };
    const d = normalizeData(v21);
    expect(d.plays.filter((p) => p.name === "Empty · Eagle").length).toBe(1);
    expect(normalizeData(JSON.parse(JSON.stringify(d))).plays.length).toBe(d.plays.length);
  });
});
